...
<%--	提供参数给 转换POST请求 的表单--%>
	<form action="workbench/clue/convert.do" method="post" id="convertForm">
		...
		<input type="hidden" name="appellation" value="${c.appellation}">
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="$('#convertForm').submit()"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
				...
				<table class="table table-hover" style="width: 900px;">
					...
					<c:forEach items="${activityList}" var="a">
						<tr>
							...
							<td>${a.endDate}</td>
							<td>${a.owner}</td>
							<td><a href="javascript:unbind('${a.id}');"  style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
...
<script type="text/javascript">
	...
	$(function(){
		...
		// 这步已经不需要,该页面(线索)关联的市场活动信息已经写进JSP了,所以不需要再发出ajax进行展示了,使用EL表达式固然方便
		// 不过会使得 这些市场活动信息 只能是全局刷新出来的,而不是通过ajax局部刷新出来的.
		// showActivityList();

		$('#qx').click(function (){
			$('.xz').prop("checked",$(this).prop("checked"))
		})
		$('#activitySearchBody').on("click",'.xz',function (){
		...
		$("#aname").keydown(function (event){
			if (event.keyCode==13){
				$.post("workbench/clue/getActivityListLikeNameAndNotBeBindedByClueId.do",{"name":$.trim($("#aname").val()),"clueId":"${c.id}"},function (data){
					var html = "";
		...
		$("#bundBtn").click(function (){
			...
			$.post("workbench/clue/bind.do",param,function (data) {
				if (data.success){
					// //关联成功，刷新列表
					// showActivityList();
					// //清除搜索框中的内容
	...
	//解除关联
	function unbind(id){
		$.post("workbench/clue/unbind.do",{id},function (data) {
			if (data.success){
				window.location.reload()
			}else{
				alert("解除关联失败")
